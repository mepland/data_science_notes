...
\begin{symbollist}
	...
	\item[HR] The hazard ratio (HR) compares the hazards of two subsets of subjects partitioned by $x$ at time $t$, $\text{HR} = \lambda\left(x_{i}\right) / \lambda\left(x_{0}\right)$. % TODO different from relative risk, also talk about log hazard
\end{symbollist}
...
Con
\begin{itemize}
	\item Limited $\lambda = c$, or $\lambda \propto t$, can be unrealistic
\end{itemize}
\end{tabular}
