...
The null hypothesis is typically that all of the groups come from populations which share the same mean.
Increased between-group variability, or decreased within-group variability,
will increase the $F$-test statistic and make rejecting the null hypothesis more probable.
One downside to ANOVA testing is that if we reject the null hypothesis
...
we can accept or reject the null hypothesis,
...
We can test the null hypothesis that the variances of two normally distributed samples are equal with a $F$-test.
...
The $F$-distribution with degrees of freedom $d_{1} = n_{1}-1$ and $d_{2} = n_{2}-1$
