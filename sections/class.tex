...
In all three cases the optimal split maximizes the decrease, \ie the gain, of relevant metric.
...
or a minimum number of training rows $\vb{x}_{i}$ at a leaf.
...
Like Gini impurity, the minimum $H = 0$ occurs when $K=1, \, p_{0}=1$, with $0 < H$ otherwise.
...
as it involves a calculating a $\log$.
