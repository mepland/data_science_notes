...
% TODO add more, and some plots too
% TODO ref in text \cref{fig:ml:supervised:early_stopping}
% TODO ref in text \cref{fig:ml:bias_variance_tradeoff}
...
= \sum_{j=1}^{n} \, \abs{\beta_{j}}\,, \label{eq:L1} \\
\Omega_{\text{L2}}\left(\bm{\beta}\right) &= \lambda \norm{\bm{\beta}}^{2}
= \sum_{j=1}^{n} \,\, \beta_{j}^{2}\,. \label{eq:L2}
\end{align}
\end{subequations}
...
% TODo Replace with pdf versions from HastieTF09 figure 3.11 someday
\begin{figure}[H]
  \centering
  \begin{subfigure}[b]{0.48\textwidth}\centering
