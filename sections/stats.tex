...
\begin{split}
\sigma_{uv}^{2} = \text{cov}\left(u,v\right) &= \lim_{N\to\infty}\frac{1}{N}\sum_{i}^{N}\left(u_{i}-\bar{u}\right)\left(v_{i}-\bar{v}\right) \\
...
% TODo replace with split up pdfs from Doug's notes
\begin{figure}
\centering
\includegraphics[width=0.95\textwidth]{figures/stats/corr_ex.png}
\caption{
